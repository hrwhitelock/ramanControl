...
class picam():
    ...
    def loadLibrary(self, pathToLib=""):
        ...
        """
        if pathToLib == "":
            pathToLib = "C:\Program Files\Common Files\Princeton Instruments\Picam\Runtime"
    ...
    def stopAcquisition(self):
        ...
        state = PicamAcquisitionStatus()
        timeout = 10000
        
        self.status(self.lib.Picam_StopAcquisition(self.cam))
        self.lib.Picam_WaitForAcquisitionUpdate(self.cam, piint(timeout), ptr(available), ptr(state))
        return

    def getAvailableFrame(self):
        available = PicamAvailableData()
        if available.readout_count >= 1:
            dataList = self.getBuffer(available.initial_readout, available.readout_count)[0:1]
            return dataList
        return []

    def getRoiWidth(self):
        rois = self.getParameter("Rois")
        w = int(np.ceil(float(rois.roi_array[0].width) / float(rois.roi_array[0].x_binning)))
        return w

    def getRoiHeight(self):
        rois = self.getParameter("Rois")
        h = int(np.ceil(float(rois.roi_array[0].height) / float(rois.roi_array[0].y_binning)))
        return h

    def waitForFrame(self, exp):

        available = PicamAvailableData()
        state = PicamAcquisitionStatus()
        timeout = int(exp+100)

        while available.readout_count < 1:
            self.lib.Picam_WaitForAcquisitionUpdate(self.cam, piint(timeout), ptr(available), ptr(state))
            if available.readout_count >= 1:
                data = self.getBuffer(available.initial_readout, available.readout_count)[0:1]
                self.lib.Picam_WaitForAcquisitionUpdate(self.cam, piint(timeout), ptr(available), ptr(state))
                return data
